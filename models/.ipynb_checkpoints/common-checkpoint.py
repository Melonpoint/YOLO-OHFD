...
class ECA(nn.Module):
    ...
    def __init__(self, c1, c2, k_size=1):
...
class AlignConv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
        # 卷积层，用于模拟对齐操作
        self.conv = nn.Conv2d(c1, c2, k, s, padding=p if p is not None else k // 2, groups=g, bias=False)
        ...
        # 对齐操作：使用卷积核对输入特征进行简单的变换
        self.alignment_conv = nn.Conv2d(c1, c1, kernel_size=3, stride=1, padding=1, groups=1, bias=False)
